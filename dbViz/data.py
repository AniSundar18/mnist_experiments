...
def _baseset_picker(args):
    ...
    if args.baseset == 'CIFAR10':
        ...
        clean_mnistm_trainset = MNISTM('./data', train=True, download=True, transform=transform_train)
        # LIAM CHANGED TO SHUFFLE=FALSE
        clean_mnistm_trainloader = torch.utils.data.DataLoader(
            clean_mnistm_trainset, batch_size=args.bs, shuffle=False, num_workers=2)
        clean_mnist_trainset = datasets.MNIST('./data', download = True, train=False, transform=transform_train)
        # LIAM CHANGED TO SHUFFLE=FALSE
        clean_mnist_trainloader = torch.utils.data.DataLoader(
        clean_mnist_trainset, batch_size=args.bs, shuffle=False, num_workers=2)
    ...
    if args.baseset == 'MNIST':
        return (clean_mnist_trainset, clean_mnistm_trainset), (clean_mnist_trainloader, clean_mnistm_trainloader), testset, testloader
...
def get_data(args):
    ...
    if args.baseset == 'MNIST':
        mnist_trainset, mnistm_trainset = clean_trainset
        mnist_trainloader, mnistm_trainloader = clean_trainloader
        
        return (mnist_trainloader, mnistm_trainloader), testloader
    else:
